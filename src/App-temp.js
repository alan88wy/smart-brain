...
class App extends Component {
    ...
    calculateFaceLocation = (data) => {
        ...
        console.log("clafai: ", clarifaiFace);
        ...
        var html = "<div className='bounding - box' style={{ top: topRow, right: rightCol, bottom: bottomRow, left: leftCol }}></div>";

        return {

            document.querySelector('inputImage').insertAdjacent('afterEnd', html);
        }
    ...
    onButtonSubmit = () => {
        ...
        fetch('https://vast-shore-64670.herokuapp.com/imageUrl', {
            ...
            .then(response => {
                ...
                const arrBox = response.outputs[0].data.regions;

                for (var i = 0; i < arrBox.length; i++) {
                    this.displayFaceBox(this.calculateFaceLocation(arrBox[0].region_info.bounding_box));

                }

                // var arrBox = response.outputs[0].data.regions;

                // console.log(typeof arrBox)
                // for (var boxArr in response.outputs[0].data.regions) {
                //     console.log(boxArr.region_info);
                // }
            })
